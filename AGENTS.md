...
- **AI calls**: Client calls only Firebase Functions through `AIService`; API keys never ship in the app. Function request payloads and AI responses are validated with Zod.
...
- **Onboarding**: The final onboarding step saves `UserProfile`, asks for AI consent when needed, then generates and saves an active workout plan if consent exists. Template choice is derived from training days.
- **Workout generation**: `generateWorkoutPlan` enforces App Check, validates input, filters by equipment, uses prompt version `2.0.0`, and applies severe/moderate/mild injury guidance.
...
- **Units**: Store all weights in kg. Convert only for display through `UnitSystem` and formatting helpers.
