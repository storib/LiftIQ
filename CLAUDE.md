...
- Warm-up sets: `WarmUpPlanner` decides prescriptions (plan's `warmUpSets`, else a synthesized 50%x8/70%x5 ramp for the first exercise of the first two straight groups; never in supersets — round-based rest pairs sets by index). `WorkoutSession.create` materializes them; `setNumber` counts within each set type. Ghost/prefill matching is set-type-aware (`previousSet`). Warm-ups rest `min(default, 60)`s and are excluded from PRs/volume/progression on both client and server.
